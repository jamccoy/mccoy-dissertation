...
First, after the silicon master was cleaned again using deionized water and isopropyl alcohol (\ce{C3H8O}), $\sim \SI{3}{\gram}$ of the primary component was dispensed through a short spin-coat process at \num{2000} rotations per \si{\minute} using a low spin acceleration, leaving a layer tens of \si{\um} thick. 
...
\begin{figure}
 ...
 \caption[A \SI{150}{\mm}-diameter, \ce{KOH}-etched, $\langle 100 \rangle$ silicon grating spin-coated with H-PDMS at the Quattrone Nanofabrication Facility (QNF).]{A \SI{150}{\mm}-diameter silicon master spin-coated with H-PDMS at Quattrone Nanofabrication Facility (QNF). Due to the high viscosity of the material, a gradual spin acceleration must be used to achieve a uniform layer.}\label{fig:UPenn_spin}
 \end{figure}
...
An identical imprint is shown under AFM in \cref{fig:SCIL_AFMs}, where the average blaze angle measures $\delta' = 27.9 \pm 0.7^{\circ}$, giving a $\delta' / \delta \approx 0.93$ or $\delta - \delta' \approx 2^{\circ}$ reduction in blaze angle relative to $\delta = 30.0 \pm 0.8^{\circ}$ measured for the silicon master [\emph{cf.\@} \cref{sec:master_for_SCIL}]. 
...
\begin{table}[]
 \centering
 \caption{Measured diffracted-arc parameters for the silicon master and the coated SCIL replica in their respective test configurations at the ALS.}\label{tab:arc_params}
 \begin{tabular}{cccc}
 \cline{1-3}
...
\begin{figure}
 ...
 \caption[Measured absolute diffraction efficiency data for the silicon master and the coated SCIL replica]{Measured absolute diffraction efficiency data for the silicon master (\emph{top}) and the coated SCIL replica (\emph{bottom}) in geometrical configurations described by the parameters listed in \cref{tab:arc_params} compared to specular reflectivity calculated using the parameters listed in \cref{tab:nom_params}.}\label{fig:abs_eff}
 \end{figure} 
...
\begin{table}[]
 \centering
 \caption{Nominal parameters relevant for specular reflectivity analysis and diffraction-efficiency modeling of the silicon master and the coated SCIL replica.}\label{tab:nom_params} 
 \begin{tabular}{cccc}
 \cline{1-3}
...
\begin{figure}
 ...
 \caption[Measured absolute diffraction efficiency data for the coated SCIL replica compared to \textsc{PCGrate-SX} models featuring two different groove shapes, each with a blaze angle of $28^{\circ}$.]{Measured absolute diffraction efficiency data for the coated SCIL replica [\emph{cf.\@} \cref{fig:abs_eff}, \emph{bottom panel}] compared to \textsc{PCGrate-SX} models featuring two different groove shapes, each with $\delta' = 28^{\circ}$, $\alpha = 30.7^{\circ}$ and $\gamma = 1.75 \pm 0.04^{\circ}$ (\emph{shaded swaths}). While the groove shape illustrated in the top panel more closely resembles the topography of the SCIL replica, it yields model results that are comparable to those produced from the simpler, ideal sawtooth (\emph{bottom panel}).}\label{fig:model_scil1} %The simpler, ideal sawtooth model thus can be used to match predicted peak-order centroids to the measured data.
 \end{figure} 
