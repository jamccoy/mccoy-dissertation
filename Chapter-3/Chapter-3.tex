...
\begin{figure}
 ...
 \caption[AFMs of the GEBL-processed resist and the resist following thermal reflow.]{AFMs of the GEBL-processed resist (\emph{top}) and the resist following thermal reflow (\emph{bottom}).}\label{fig:TASTE_AFMs}
 \end{figure}
...
\begin{figure}
 ...
 \caption[Fresnel reflectivity and penetration depth in a thick slab of gold for nominal test geometry]{Fresnel reflectivity, $\mathcal{R}_F$, and penetration depth, $\mathcal{D}_{\perp}$, in a thick slab of gold at $\zeta = 1.7^{\circ}$ over $\SI{80}{\electronvolt} \leq  \mathcal{E}_{\gamma} \leq \SI{800}{\electronvolt}$ (data obtained from CXRO \cite{CXRO_database}).}\label{fig:Au_refl_attn} %($\mathcal{R}_F$ given by \cref{eq:ch3_fres_refl}) %($\mathcal{D}_{\perp}$ given by \cref{eq:ch3_pene_dep}) 
 \end{figure}
...
\begin{figure}
 ...
 \caption[AFM of the grating prototype grooves following electron-beam physical vapor deposition of gold, using titanium as an adhesion layer on PMMA]{AFM of the grating prototype grooves following EBPVD of gold, using titanium as an adhesion layer on PMMA.}\label{fig:coating_AFM}
 \end{figure}
...
\begin{figure}
 ...
 \caption[FESEM of the gold-coated grating prototype grooves viewed top-down]{Field-emission scanning electron micrograph (FESEM) of the gold-coated grating prototype grooves viewed top-down, taken with a \textsc{Zeiss Leo 1530} instrument at the Penn State Nanofabrication Laboratory.}\label{fig:coating_SEM}
 \end{figure}
...
Moreover, the bottom plateau of the coated grooves appears slightly widened relative to the bottom plateau of the uncoated grooves, %bare, TASTE-processed resist,
where the surface of the silicon substrate is exposed; this suggests that the EBPVD process utilized produces a non-uniform coating, which is likely due to the geometry of the EBPVD chamber coupled with the topography of the grating grooves. 
%a thicker metal coating on a silicon surface with native \ce{SiO2} than it does on PMMA resist. 
...
\begin{figure}
 ...
 \caption[Photograph of the TASTE grating prototype installed inside the test chamber of ALS beamline 6.3.2.]{The TASTE grating prototype [\emph{cf.\@} \cref{sec:prototype_TASTE_process,sec:prototype_coating}] installed inside the test chamber of ALS beamline 6.3.2.}\label{fig:test_chamber} 
 \end{figure}
...
\begin{table}[]
\centering
\caption{Measured parameters for the test-configuration diffracted arc of the TASTE grating prototype at the ALS.}\label{tab:arc_params_taste}
\begin{tabular}{@{}ll@{}}
\toprule
